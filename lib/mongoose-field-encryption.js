...
const encryptionEnabled = process.env.ENCRYPTION_ENABLED && JSON.parse(process.env.ENCRYPTION_ENABLED);
...
const decryptFields = function (obj, fields, secret) {
    ...
    if (obj[encryptedFieldName]) {
      if (obj[encryptedFieldData]) {
        ...
        if (obj[field]) {
          ...
          obj[encryptedFieldName] = false;
          if(obj[field] !== decryptedValue){
            obj._doc[field] = decryptedValue;
            obj = obj._doc;
          }
        }
      }
  ...
  if(Array.isArray(data)){
    data.forEach(objData => {
        if(typeof objData == 'object'){
          decryptData(objData, secret);
        }
    })
} else if(typeof data == 'object'){
    for(const key of Object.keys(data)){
        if(data[key] && (Array.isArray(data[key]) || typeof data[key] == 'object')){
          decryptData(data[key], secret);
        }
    }
    const fieldsToDecrypt = [];
    Object.keys(data).forEach(k =>{
      if(k.startsWith(encryptedFieldNamePrefix)){
        fieldsToDecrypt.push(k.replace(encryptedFieldNamePrefix, ''))
      }
    });
    decryptFields(data, fieldsToDecrypt, secret);
  }
}

const decryptMixFields = function (data, secretKey){
  if(!encryptionEnabled) return;
  const _hash = (secret) => crypto.createHash("sha256").update(secret).digest("hex").substring(0, 32);
  const secret = _hash(secretKey);
...
const fieldEncryption = function (schema, options) {
  ...
  function decryptFields(obj, fields, secret) {
    for (const field of fields) {
      ...
      if (obj[encryptedFieldName]) {
        if (obj[encryptedFieldData]) {
          const encryptedValue = obj[encryptedFieldData];
          const decryptedValue = JSON.parse(decrypt(encryptedValue, secret));
          obj[field] = decryptedValue
          obj[encryptedFieldName] = false;
          obj[encryptedFieldData] = "";
          if(obj[field] !== decryptedValue){
  ...
  schema.post("init", function (_next, _data) {
    ...
    try {
      if(encryptionEnabled) decryptFields(data, fieldsToEncrypt, secret());
      next();
    } catch (err) {
      next(err);
    }
  ...
  schema.pre("save", function (_next) {
    const next = getCompatitibleNextFunc(_next);
    try {
      if(encryptionEnabled) encryptFields(this, fieldsToEncrypt, secret());
      next();
    } catch (err) {
      next(err);
    }
...
module.exports.decryptMixFields = decryptMixFields;
